...
int CDevice::getConfig(int item, int* value)
{
	if (!this->isOpen())
	{
...
int CDevice::getLatest(int channel, int label, void *data, int *seq_num)
{
	if (!this->isOpen())
	{
		return ARS_WINRTFAIL;
...
// 获取Arinc429波特率
int CDevice::getBaudRate(int channel, int trx, int* value)
{
...
// @param trx
// @param value
int CDevice::getParity(int channel, int trx, int* value)
{
	ULONG regAddr = ARINC_CORE_ADDR(channel, trx, ARINC_REG_CTRL_ID);
	ULONG regVal;
...
// @param channel 
// @param value 
int CDevice::getDecoder(int channel, int* value)
{
...
// @param channel 
// @param value 
int CDevice::getSDI(int channel, int* value)
{
	ULONG regAddr = ARINC_RXREG_ADDR(channel, ARINC_REG_CTRL_ID);
	ULONG regVal;
...
// @param channel 
// @param value 
int CDevice::getLookBack(int channel, int* value)
{
