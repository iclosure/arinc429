...
c51 tcxmaster.c db oe code small moddp2 noiv
...
echo fw.obj, dscr.obj, tcxmaster.obj, > tmp.rsp
echo %CYUSB%\target\lib\FX2\USBJmpTb.obj, >> tmp.rsp
echo %CYUSB%\target\lib\FX2\EZUSB.lib  >> tmp.rsp
if "%1" == "-i" echo TO tcxmaster RAMSIZE(256) PL(68) PW(78) CODE(80h) XDATA(1800h)  >> tmp.rsp
if not "%1" == "-i" echo TO tcxmaster RAMSIZE(256) PL(68) PW(78) CODE(4000h) XDATA(5800h)  >> tmp.rsp
bl51 @tmp.rsp

REM ### Generate intel hex image of binary (no debug info) ###
oh51 tcxmaster HEXFILE(tcxmaster.hex)

REM ### Generate serial eeprom image for C2 (EZ-USB FX2) bootload ###
if "%1" == "-i" %CYUSB%\bin\hex2bix -i -f 0xC2 -c 0xC9 -o tcxmaster.iic tcxmaster.hex
